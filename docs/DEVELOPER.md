...
The technical detail behind [BorderShield AI](../README.md): how the Trust Ladder is wired, where each
module lives, how the real-document mode works, how it is deployed and tested. The README is the
plain-language overview; this file is for anyone who is going to read or change the code.

Everything here reflects the code as it stands on `master`. If a background document in `docs/01`-`06`
disagrees with this file or the code, the code wins (those documents predate the final scope cut).
...
3. Nothing else to configure: `requirements.txt` and `packages.txt` (system libs — `libgl1` /
   `libglib2.0-0t64`, needed for `opencv-contrib-python` on the Linux container — the plain
   `libglib2.0-0` name conflicts on Streamlit Cloud's current base image) are both already
...
4. The demo corpus (`data/documents/`, `data/forged/`) and models (`models/*.onnx`, `models/*.npz`)
   ...
   local generation step. `data/pki/` (signing keys) stays gitignored. At startup `synth.sign.ensure_corpus_signed()` creates a demo
   PKI on that machine and re-signs the demo documents if their signatures don't verify there; the issuer
   registry's `manifest.json` is likewise signed per machine. That is by design, not a missing step.
5. First boot installs `opencv-contrib-python`/`onnxruntime`/`rapidocr-onnxruntime` from
   ...
   (cached).
6. The live instance auto-redeploys on every push to `master`.

## Architecture — the Trust Ladder

```
T0  Cryptographic signature        decisive BOTH ways       core/crypto/
T1  MRZ + rules + cross-zone       decisive AGAINST only    core/mrz.py, core/rules/, core/crosszone.py
T1  Issuer provenance (synthetic)  decisive AGAINST only    core/issuer/
T2  Forensics                      advisory, caps at HIGH   core/forensics/
T2  Biometric face match           advisory, caps at HIGH   core/face/
T2  Identity continuity            advisory, caps at HIGH   core/issuer/ (biometric-reference linkage)
                     ↓
              core/risk.py fuse() → Verdict (score, band, action)
```

Fusion is tier-first, not a weighted sum. Two hard overrides sit above the score: an invalid signature
is CRITICAL/100, and any failure in a *decisive* tier (Rules, Issuer) floors the verdict at CRITICAL 76.
Advisory tiers can add score but can never reach CRITICAL on their own. Bands: LOW 0, MEDIUM 1–50,
...
| Path | What's in it |
|---|---|
| `core/` | Mode A's pipeline: MRZ/VIZ reading, crosszone, rules engine, forensics, face verification, crypto (PKI + signed manifest + ledger with truncation checkpoint), risk fusion. |
| `core/issuer/` | The synthetic issuer registry: SQLite store, signed integrity manifest, an `IssuerRegistryProvider` interface a real connector could implement, and the identity-continuity linkage. |
| `core/knowledge/` | `standards.py` — the "why is this required?" citations (real external standards where one applies, an honest "our own policy" where it doesn't). |
...
| `data/pki/`, `data/portraits/`, `data/registry/manifest.json`, `results/` | Gitignored. `data/pki/` holds real (if demo-only) private keys; `data/portraits/` holds real consenting-person photos; `results/` is session-local ledger output. |

## The console

6 screens, fixed sidebar nav (visuals: the Sovereign light design system — flat, warm-white, one terracotta accent, semantic green/amber/red only for verdicts):

| Screen | Shows |
|---|---|
| **Overview** | The thesis, what the system does and doesn't do yet, the Trust Ladder drawn as a connected diagram, and the detail behind those limits — read once, before operating the console. |
| **Command** | 5 real status cards (models / PKI / issuer registry / ledger / cases), the 13-scenario Attack Wall as scenario cards, a registry-integrity demo, recent cases. |
| **New screening** | Mode toggle: **Demo Document** (the UTO template) or **Real Document** (any arbitrary upload, see below), with an optional second document for a date-of-birth cross-check. |
| **Case file** | One case, one page: verdict, the six-row ladder that produced it, findings, score contributions, the pipeline log, MRZ, extracted identity, the registry lookup, identity continuity, "why is this required?" citations, JSON export. |
| **Audit trail** | Hash-chained ledger records across all cases, two independent integrity checks (chain + signed truncation checkpoint), tamper and truncation demos, JSON export. |
| **System status** | Models, demo signing PKI, `policy.yaml` weights/bands/overrides, ledger state, live test count, and the full issuer-registry contents with search — read directly off this machine. |

### Attack Wall — what each scenario actually does

| Scenario | Attack | Tier that catches it |
|---|---|---|
| SCN_01 Genuine document | Untouched document | Everything passes → LOW |
| SCN_02 Change of birth date | VIZ date of birth edited, MRZ left alone | T1 cross-zone mismatch → CRITICAL |
| SCN_03 Replace the portrait | Portrait swapped with a feathered seam | T2 forensics + T0 crypto (impersonation mode) |
| SCN_04 Screen recapture | Real re-encode + moiré/glare simulation | T2 forensics only — routes to review, never CRITICAL |
| SCN_05 Face mismatch | *(disabled — see below)* | T2 biometric |
| SCN_06 Break the signature | Hand-tampers an already-signed manifest | T0 crypto — CRITICAL, zero forensic/biometric input consulted |
| SCN_07 Unregistered document | Document number the registry has never seen | T1 issuer — reported as "issuance could not be established", never as fake |
| SCN_08 Perfect document, revoked | Self-consistent, cleanly-signed document; the registry marks it REVOKED | T1 issuer — no pixel is forged, nothing else fires |
| SCN_09 Stolen document | Same, registry marks it STOLEN | T1 issuer |
| SCN_09B Registry record expired | Document's own printed dates are current; the registry's record is EXPIRED | T1 issuer |
| SCN_09C Registry record invalid | Never validly standing (distinct from REVOKED) | T1 issuer |
| SCN_10 Valid number, wrong identity | A real ACTIVE document number under a name/DOB the registry disagrees with | T1 issuer — field mismatch |
| SCN_11 Same person, multiple identities | Clean record, but its biometric reference is also on file under a different identity | T2 identity continuity — advisory, capped at HIGH |
...
| Cross-document consistency | Optional second upload (`core/realdoc/crossdoc.py`) — screens both documents independently, then compares date of birth. Only `EXTRACTED`-confidence reads are compared (never `UNCERTAIN`); a value neither/one side confidently read is reported as "could not be compared", not guessed. A conflict re-fuses into the primary document's score; agreement costs nothing. This is the actual mechanism behind "verify a DOB without a government database" — two independently-issued documents agreeing is real corroborating evidence. |
...
**Team documents** (self-contained HTML — download and open in any browser; GitHub shows the source, not the page):

| Doc | Who it's for |
|---|---|
| [`docs/team/dossier.html`](team/dossier.html) | **Everyone.** Answers any question a judge can ask, in plain English — what the project is, the three questions people mix up, what each screen shows, easy and hard question banks, glossary. Read once before Round 2. |
| [`docs/team/run-of-show.html`](team/run-of-show.html) | **Whoever presents.** What each screen is, then the click-by-click demo script: what to click, what to say, how long, and what to do if something breaks live. |
| [`docs/team/current-state.md`](team/current-state.md) | **Whoever's planning the final build.** Every screen and button, what's real vs. disabled vs. not built, and why — the baseline before adding anything new. |
...
These docs predate the final scope cut (they still describe an earlier, larger vision); the code in `core/`/`ui/` and this file are the current source of truth whenever they disagree.
